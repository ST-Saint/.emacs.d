...
variable `project-local-identifier' to be considered a project."
          ...
          (thread-last
            ...
            (seq-sort (pcase-lambda (`(,f1 . ,t1) `(,f2 . ,t2))
                        ...
                        (cond ((or (not (file-exists-p f1))
                              ...
                              (t (time-less-p t2 t1)))))
            (mapcar #'car))
        ;; nil: project, need to expand the full path
        (thread-last
          files
          (mapcar (lambda (f) (concat (project-root (project-current)) f)))
          (mapcar (lambda (f)
                    (cons f (file-attribute-modification-time (file-attributes f)))))
          (seq-sort (pcase-lambda (`(,f1 . ,t1) `(,f2 . ,t2))
                      ;; Want existing, most recent, local files first.
  ...
  ;; TODO use recentf instead of modification time
