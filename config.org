...
(use-package project
  ...
  (setq project-switch-commands '((project-find-file "File file" ? )
...
variable `project-local-identifier' to be considered a project."
  ...
  (defun filepath-equal (p1 p2)
    (equal (expand-file-name p1) (expand-file-name p2)))

  (defun project-realpath (filepath)
    (if (or (string-prefix-p "/" filepath)
            (or (string-prefix-p "~/" filepath)
                (file-directory-p filepath)))
        filepath
      (concat (if (project-current) (project-root (project-current)) nil) filepath)))

  (defun project-nth-recentf (file)
    (or (cl-position (project-realpath file)
                     recentf-list :test #'filepath-equal)
        65536))

  (defun project-sort-files (files)
    (if (< (length files) 1024)
        (mapcar #'car
                (seq-sort (lambda (tp1 tp2)
                            ;; Want existing, most recent, local files first.
                            (cl-destructuring-bind (f1 rp1 rk1 t1) tp1
                              (cl-destructuring-bind (f2 rp2 rk2 t2) tp2
                                (cond ((not (file-exists-p rp1)) nil)
                                      ((not (file-exists-p rp2)) t)
                                      ((not (equal rk1 rk2)) (< rk1 rk2))
                                      (t (time-less-p t2 t1))))))

                          (if (< (length files) 256)
                              (mapcar (lambda (f)
                                        (list f (project-realpath f) (project-nth-recentf f) (file-attribute-modification-time (file-attributes (project-realpath f))))) files)
                            (mapcar (lambda (f)
                                      (list f (project-realpath f) 0 (file-attribute-modification-time (file-attributes (project-realpath f))))) files)
                            )))
      files))

  ;; TODO improve complexity
  (setq vertico-multiform-commands
        '((project-find-file (vertico-sort-override-function . project-sort-files))
          (project-switch-project (vertico-sort-override-function . project-sort-files))))
...
(use-package persp-mode
  ...
  ;;       (mapcar 'persp-name (sort persps (lambda (p1 p2)
  ;;                                          (time-less-p (persp)))))
  ;;       ))
  ...
  (defun persp-switch-last ()           ; FIXME record real last
    (interactive)
...
(use-package yasnippet
  ...
  (with-eval-after-load 'org
      (add-to-list 'yas-snippet-dirs (concat org-directory "yasnippets")))
