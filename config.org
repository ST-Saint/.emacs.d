...
(use-package agda2
  ...
  (defun agda-setup-cape ()
    (with-eval-after-load 'cape
      ...
      (setq cape-agda-prefix-required t
            cape-agda-prefix '(?\\)
            cape-agda-prefix-regex (concat (regexp-opt (mapcar #'char-to-string cape-agda-prefix)) "[^ \n\t]*"))


      (defvar cape--agda-properties
        (list :annotation-function (apply-partially #'cape-char--annotation cape--agda-hash)
              :company-docsig (apply-partially #'cape-char--signature cape--agda-hash)
              :exit-function (apply-partially #'cape-char--exit cape--agda-hash)
              :company-kind (lambda (_) 'text)
              :exclusive 'no)
        "Completion extra properties for cape-agda.")

      (defun cape-agda (&optional interactive)
        (interactive (list t))
        (if interactive
            (let* ((completion-cycle-threshold cape-agda-prefix-required))
              (when (and (memq last-input-event cape-agda-prefix)
                         (not (looking-back cape-agda-prefix-regex (pos-bol))))
                (self-insert-command 1 last-input-event))
              (cape-interactive #'cape-agda))
          (when-let ((bounds
                      (cond
                       ((looking-back cape-agda-prefix-regex (pos-bol))
                        (cons (match-beginning 0) (point)))
                       ((not cape-agda-prefix-required) (cons (point) (point))))))
            (append
             (list (car bounds) (cdr bounds)
                   (cape--properties-table cape--agda-hash :category 'cape-agda))
             cape--agda-properties))))

      (deactivate-input-method)
      (add-to-list 'completion-at-point-functions #'cape-agda t)))
